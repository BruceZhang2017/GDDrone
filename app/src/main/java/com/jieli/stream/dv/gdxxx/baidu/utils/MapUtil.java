package com.jieli.stream.dv.gdxxx.baidu.utils;
...
import static com.jieli.stream.dv.gdxxx.baidu.utils.BitmapUtil.bmArrowPoint;
import static com.jieli.stream.dv.gdxxx.baidu.utils.BitmapUtil.bmEnd;
import static com.jieli.stream.dv.gdxxx.baidu.utils.BitmapUtil.bmStart;
...
public class MapUtil {
     ...
     * @param trackApp
     */
 /*   public void setCenter(a trackApp) {
    ...
    public void updateStatus(LatLng currentPoint, boolean showMarker) {
        ...
        if (null != baiduMap.getProjection()) {
            ...
            if (screenPoint.y < 200 || screenPoint.y > a.screenHeight - 500
                    || screenPoint.x < 200 || screenPoint.x > a.screenWidth - 200
                    || null == mapStatus) {
...
//        OverlayOptions endOptions = new MarkerOptions().position(endPoint).icon(bmEnd).zIndex(9).draggable(true);
        ...
        // 添加路线（轨迹）
        OverlayOptions polylineOptions = new PolylineOptions().width(10).color(Color.rgb(30, 144, 255)).points(points);

        baiduMap.addOverlay(startOptions);
//        baiduMap.addOverlay(endOptions);
        polylineOverlay = baiduMap.addOverlay(polylineOptions);

        OverlayOptions markerOptions = new MarkerOptions().flat(true).anchor(0.5f, 0.5f).icon(bmArrowPoint)
    ...
    public void animateMapStatus(List<LatLng> points) {
        if (null == points || points.isEmpty()) {
            Dbug.e(tag, "animateMapStatus:null error");
            return;
        }
        ...
        if (baiduMap != null) baiduMap.animateMapStatus(msUpdate);
    }

    public void animateMapStatus(LatLng point, float zoom) {
        MapStatus.Builder builder = new MapStatus.Builder();
        mapStatus = builder.target(point).zoom(zoom).build();
        if (baiduMap != null) baiduMap.animateMapStatus(MapStatusUpdateFactory.newMapStatus(mapStatus));
    }

    public void setMapStatus(LatLng point, float zoom) {
        MapStatus.Builder builder = new MapStatus.Builder();
        mapStatus = builder.target(point).zoom(zoom).build();
        if (baiduMap != null) baiduMap.setMapStatus(MapStatusUpdateFactory.newMapStatus(mapStatus));
    }
    ...
    public void addEndPointOverlay(List<LatLng> points) {
        if (points != null && points.size() > 0) {
            LatLng endPoint = points.get(points.size() - 1);
            OverlayOptions endOptions = new MarkerOptions().position(endPoint).icon(bmEnd).zIndex(9).draggable(true);
            if (baiduMap != null) baiduMap.addOverlay(endOptions);
        }
    }
