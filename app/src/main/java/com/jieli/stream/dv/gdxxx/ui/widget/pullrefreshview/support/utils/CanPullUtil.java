...
import com.jieli.stream.dv.gdxxx.ui.widget.pullrefreshview.support.impl.Pullable;
...
public class CanPullUtil {
        ...
        public boolean isGetTop() {
            if(absListView != null){
                if (absListView.getCount() == 0) {
                    ...
                    if(view != null && view.getTop() >= absListView.getPaddingTop()){
                        return true;
                    }
                }
            }
    ...
    private static class ScrollViewCanPull implements Pullable {
        ...
        @Override
        public boolean isGetBottom() {
            return scrollView != null && (scrollView.getChildCount() == 0 || null != scrollView.getChildAt(0)
                    && scrollView.getScrollY() >= (scrollView.getChildAt(0).getHeight() - scrollView.getMeasuredHeight()));
        }
    }
        ...
        public boolean isGetTop() {
            ...
            if (layoutManager != null) {
                if (layoutManager.getItemCount() == 0) {
                    return true;
                } else if (layoutManager.findFirstVisibleItemPosition() == 0 && null != recyclerView.getChildAt(0) && recyclerView.getChildAt(0).getTop() >= recyclerView.getPaddingTop()) {
                    return true;
                }
            }
            return false;
