...
public class CoverFlowLayoutManger extends RecyclerView.LayoutManager {
    ...
    private void layoutItems(RecyclerView.Recycler recycler,
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (mAllItemFrames.get(position) != null && !Rect.intersects(displayFrame, mAllItemFrames.get(position))) {//Item没有在显示区域，就说明需要回收
        ...
        for (int i = 0; i < getItemCount(); i++) {
            if (mAllItemFrames.get(i) != null && Rect.intersects(displayFrame, mAllItemFrames.get(i)) &&
...
//            cm.setSaturation(0.9f);
    ...
    public void onScrollStateChanged(int state) {
        ...
        switch (state) {
            case RecyclerView.SCROLL_STATE_IDLE:
                //滚动停止时
                Dbug.i(tag, "scroll_state_idle");
                fixOffsetWhenFinishScroll();
                break;
            case RecyclerView.SCROLL_STATE_DRAGGING:
                //拖拽滚动时
                Dbug.i(tag, "scroll_state_dragginf");
                break;
            case RecyclerView.SCROLL_STATE_SETTLING:
                Dbug.i(tag, "scroll_state_settling");
                //动画滚动时
                break;
        }
    }
    ...
    private void onSelectedCallBack() {
        mSelectPosition = Math.abs(Math.round(mOffsetAll / getIntervalDistance()));
    ...
    public void setSelectPosition(int position) {
        startScroll(mSelectPosition, position);
    }
